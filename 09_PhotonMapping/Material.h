...
public:
	...
	Material(const Vec3& ambient, const Vec3& diffuse, const Vec3& specular, float exponent, float local, const std::optional<float>& IOR, bool isShadowCaster, const Vec3& emission = Vec3{})
		: ambient(ambient), diffuse(diffuse), specular(specular), emission(emission), exponent(exponent), local(local), IOR(IOR), m_isShadowCaster(isShadowCaster)
	{ }
